...
import { subscribeToInquiries, InquiryItem } from '../lib/firebase';
...
export default function AdminPanel() {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'stats' | 'projects' | 'skills' | 'experience' | 'services' | 'inquiries' | 'backup'>('profile');
  ...
  // Subscribe to real-time incoming inquiries from Firebase Firestore
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      ...
      >
        {/* Top Header */}
        <div className="px-6 py-4 bg-[#15151D] border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sticky top-0 z-20">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-[#7C3AED] to-[#06B6D4] flex items-center justify-center text-white shadow-lg shadow-[#7C3AED]/20 shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-mono bg-emerald-500/15 text-emerald-400 border border-emerald-500/30">
                  ...
                  Firebase Firestore Cloud Connected
                </span>
              </div>
              <p className="text-xs text-[#94A3B8]">
                Sinkronisasi database awan Firebase Firestore aktif. {firebaseLastSync && `(Terakhir sync: ${firebaseLastSync})`}
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2 self-end sm:self-auto">
            {/* Quick Firebase Cloud Sync Button */}
            <button
              onClick={() => syncToCloudFirestore()}
              disabled={isFirebaseSyncing}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#06B6D4]/15 hover:bg-[#06B6D4]/25 text-[#06B6D4] border border-[#06B6D4]/30 text-xs font-semibold active:scale-95 transition-all disabled:opacity-50"
              title="Simpan & Sinkronkan ke Firebase Firestore"
            >
        ...
        <div className="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-6">
          ...
          {/* 7. INQUIRIES / PESAN MASUK TAB (FIRESTORE) */}
          {activeTab === 'inquiries' && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-[#15151D] border border-white/10">
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className="text-sm font-bold text-white">Daftar Pesan & Permintaan Kargo Masuk</h4>
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-mono bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                      Real-time Firestore
                    </span>
                  </div>
                  <p className="text-xs text-[#94A3B8]">
                    Setiap formulir kontak yang dikirim oleh calon pengirim/pabrik akan otomatis masuk ke sini secara langsung.
                  </p>
                </div>
                <div className="text-xs text-[#06B6D4] font-semibold bg-[#06B6D4]/10 px-3 py-1.5 rounded-xl border border-[#06B6D4]/20 self-start sm:self-auto">
                  Total: {inquiries.length} Pesan
                </div>
              </div>

              {inquiries.length === 0 ? (
                <div className="p-12 text-center rounded-2xl bg-[#15151D]/60 border border-white/5 space-y-3">
                  <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mx-auto text-[#94A3B8]">
                    <Inbox className="w-6 h-6" />
                  </div>
                  <h5 className="text-sm font-semibold text-white">Belum Ada Pesan Masuk</h5>
                  <p className="text-xs text-[#94A3B8] max-w-md mx-auto">
                    Ketika pengunjung mengisi form pemesanan kargo di website, data akan otomatis tercatat di database Firebase Firestore dan tampil di sini secara real-time.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {inquiries.map((inq) => (
                    <div
                      ...
                      className="p-5 rounded-2xl bg-[#15151D] border border-white/10 hover:border-white/20 transition-all space-y-3 shadow-lg"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/5 pb-3">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-bold text-white">{inq.name}</span>
                            <span className="px-2 py-0.5 rounded text-[10px] bg-[#7C3AED]/20 text-[#06B6D4] border border-[#7C3AED]/30 font-medium">
                              {inq.subject || 'Permintaan Kargo'}
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          {inq.phone && (
                            <a
                              href={`https://wa.me/${inq.phone.replace(/[^0-9]/g, '')}?text=Halo%20${encodeURIComponent(inq.name)},%20saya%20Alin%20Dwi%20Ramadhan%20(Driver%20Tangki%20PT%20Budi%20Starch)%20menerima%20pesan%20kargo%20Anda.`}
                              target="_blank"
                              rel="noreferrer"
                          ...
                          <a
                            href={`mailto:${inq.email}?subject=Tanggapan:%20${encodeURIComponent(inq.subject)}`}
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 border border-white/10 text-xs font-medium transition-colors"
                          >
                      ...
                      {/* Message Content */}
                      <div className="p-3.5 rounded-xl bg-[#111118] border border-white/5 text-xs text-slate-200 leading-relaxed">
                        {inq.message}
                      </div>
                    </div>
                  ))}
          ...
          {/* 8. BACKUP & CLOUD FIRESTORE TAB */}
          {activeTab === 'backup' && (
            <div className="space-y-6">
              {/* Firebase Cloud Firestore Sync Card */}
              <div className="p-5 rounded-2xl bg-gradient-to-br from-[#15151D] to-[#1a1a26] border border-[#7C3AED]/30 space-y-4 shadow-xl">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <div className="w-9 h-9 rounded-xl bg-[#7C3AED]/20 border border-[#7C3AED]/40 flex items-center justify-center text-[#06B6D4]">
                      <Cloud className="w-5 h-5" />
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-white">Database Cloud Firebase Firestore</h4>
                  ...
                  <span className="text-[11px] text-[#94A3B8]">
                    {firebaseLastSync ? `Terakhir sync: ${firebaseLastSync}` : 'Belum sync manual'}
                  </span>
                </div>

                <p className="text-xs text-[#94A3B8] leading-relaxed">
                  Data portofolio (profil, foto, rute pengiriman, statistik, dan keahlian) tersimpan secara aman di Google Firebase Firestore Cloud Database, sehingga dapat diakses secara persisten dari perangkat mana pun.
                </p>
                ...
                <div className="flex flex-wrap gap-3 pt-1">
                  <button
                    onClick={() => syncToCloudFirestore()}
                    disabled={isFirebaseSyncing}
                    className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#7C3AED] to-[#06B6D4] text-white text-xs font-semibold shadow hover:opacity-95 active:scale-95 transition-all disabled:opacity-50"
                  >
                    <RefreshCw className={`w-4 h-4 ${isFirebaseSyncing ? 'animate-spin' : ''}`} />
                    <span>{isFirebaseSyncing ? 'Menyinkronkan...' : 'Simpan / Sync ke Cloud Sekarang'}</span>
                  </button>

                  <button
                    onClick={() => syncFromCloudFirestore()}
                    disabled={isFirebaseSyncing}
                    className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/15 text-white text-xs font-semibold border border-white/10 active:scale-95 transition-all disabled:opacity-50"
                  >
              ...
              <div className="p-5 rounded-2xl bg-[#15151D] border border-white/10 space-y-4">
                <h4 className="text-sm font-bold text-white">Cadangan File JSON Lokal</h4>
                <p className="text-xs text-[#94A3B8] leading-relaxed">
                  Unduh seluruh konfigurasi portofolio Anda sebagai file JSON mandiri untuk arsip offline atau reset data ke setelan pabrik.
                </p>

                <div className="flex flex-wrap gap-3">
                  <button
                    onClick={exportDataJSON}
                    className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/15 text-white text-xs font-semibold border border-white/10"
                  >
                    <Download className="w-4 h-4" />
